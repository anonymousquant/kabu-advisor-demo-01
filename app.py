...
if name:
    ...
    if st.button("Xで話題の銘柄を見る"):
        prompt = """
        X（旧Twitter）で直近話題になっている日本株を5銘柄挙げてください。
        ...
        try:
            ...
            st.error(f"Geminiへの接続に失敗しました。しばらく待ってから再試行してください。\n\n({e})")
            st.stop()
        ...
        # session_stateに保存 ← ここが核心
        st.session_state.matches = matches

    # session_stateから表示（ボタンを押していなくても残る）
    ...
    if ticker_input:
        ...
        if price is not None:
            sign = "+" if change_pct >= 0 else ""
            st.write(f"**{stock_name}（{ticker_input}）**：{price:,.0f}円　（前日比 {sign}{change_pct:.2f}%）")
